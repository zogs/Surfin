(function() {
    ...
    function Shield(conf = {}) {
      ...
      let defaults = {
        ...
        radius: 80,
        speed: 200,
        color: "rgba(75,175,216,0.7)",
      }
      this.conf = Object.assign({}, defaults, conf);
      this.conf.scale = this.conf.scale || 1;
    ...
    Shield.prototype.init = function(conf) {
      ...
      this.circle.graphics.setStrokeStyle(5).beginStroke('rgba(255,255,255,0.2)').beginRadialGradientFill(["rgba(0,0,0,0)",this.conf.color], [0, 1], 0, 0, 0, 0, 0, radius).drawCircle(0,0,radius);
    ...
    Shield.prototype.open = function() {
      this.isOpen = true;
      this.isActive = true;
      createjs.Tween.get(this).to({scale: this.conf.scale, alpha:1}, this.conf.speed, createjs.Ease.bounceOut);
